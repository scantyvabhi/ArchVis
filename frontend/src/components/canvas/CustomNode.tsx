...
export const CustomNode = memo(({ data, selected }: NodeProps<ArchitectureNode>) => {
  ...
  if (isOverloaded) {
    ...
    containerBorder = 'border-slate-800 ring-2 ring-slate-400';
  }

  // Handle styles
  const handleStyle = '!w-2.5 !h-2.5 !bg-slate-400 !border-2 !border-white hover:!bg-blue-600 transition-colors';

  // LOW ZOOM VIEW (< 0.5x)
  if (isLowZoom) {
    return (
      <div
        className={`relative rounded-xl border bg-white px-3 py-2 transition-all ${containerBorder} ${containerShadow} ${pulseClass} min-w-[140px] flex items-center gap-2`}
      >
        <Handle type="target" position={Position.Top} className={handleStyle} />
        <Handle type="target" position={Position.Left} className={handleStyle} />

        <div className={`p-1.5 rounded-lg ${style.iconBg} ${style.iconColor} shrink-0`}>
          <IconComponent className="w-4 h-4" />
        </div>
        <div className="truncate">
          <p className="font-semibold text-xs text-slate-800 truncate">{data.label}</p>
          <span className="text-[10px] text-slate-500 uppercase tracking-wider font-mono">{data.tech || category}</span>
        </div>
  ...
  return (
    <div
      className={`group relative rounded-xl border bg-white transition-all ${containerBorder} ${containerShadow} ${pulseClass} min-w-[260px] max-w-[300px] overflow-hidden`}
    >
      ...
      {/* Top Header Card */}
      <div className="p-3 bg-gradient-to-b from-slate-50/80 to-white border-b border-slate-100 flex items-start justify-between gap-2">
        <div className="flex items-center gap-2.5">
          <div className={`p-2 rounded-lg ${style.iconBg} ${style.iconColor} transition-transform group-hover:scale-105`}>
            <IconComponent className="w-4 h-4" />
          </div>
          <div>
            <h3 className="font-semibold text-sm text-slate-800 leading-tight">{data.label}</h3>
            <span className="inline-block font-mono text-[11px] text-slate-500 font-medium">
              {data.tech || category}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 shrink-0">
          {isOverloaded ? (
            ...
            <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded border uppercase tracking-wider font-mono ${style.bgBadge} ${style.textBadge}`}>
              {category}
            </span>
          )}
        </div>
      ...
      <div className="p-3 space-y-2.5">
        ...
        <div>
          <div className="flex items-center justify-between text-[11px] mb-1">
            <span className="text-slate-500 font-medium">Traffic / Capacity</span>
            <span className="font-mono font-semibold text-slate-700">
              {data.rps?.toLocaleString() || 0} <span className="text-slate-400 font-normal">/ {data.capacity?.toLocaleString()} rps</span>
            </span>
          </div>
          ...
          <div className="pt-2 border-t border-slate-100 space-y-2 text-[11px]">
            <div className="grid grid-cols-2 gap-2 text-slate-600">
              <div className="bg-slate-50 p-1.5 rounded border border-slate-100">
                <span className="block text-[9px] text-slate-400 uppercase font-mono">Latency</span>
                <span className="font-mono font-medium text-slate-800">
                  {data.effectiveLatency ?? data.latency ?? 20} ms
                </span>
              </div>
              <div className="bg-slate-50 p-1.5 rounded border border-slate-100">
                <span className="block text-[9px] text-slate-400 uppercase font-mono">Fail Rate</span>
                <span className="font-mono font-medium text-slate-800">
            ...
            {data.replication && (
              <div className="bg-slate-50 p-1.5 rounded border border-slate-100 text-[10px]">
                <span className="block text-[9px] text-slate-400 uppercase font-mono">Replication</span>
                <span className="text-slate-700 font-medium truncate block">{data.replication}</span>
              </div>
            )}

            {data.explanation && (
              <div className="p-2 rounded bg-blue-50/60 border border-blue-100 text-[10px] text-slate-600 leading-relaxed">
                <span className="font-semibold text-blue-800 block mb-0.5">Architectural Note:</span>
                {data.explanation}
              </div>
            )}
          </div>
